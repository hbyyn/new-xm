  ...
  methods: {
    ...
    queryStringId(queryString, cb) {
      let restaurants = (() => {
        ...
        for (var i = 0; i < this.mock_all.list.length; i++) {
          ...
          restaurants[i].value = this.mock_all.list[i].customers_id//修改点
        }
        // 去重
        let hash = {};
    ...
    queryStringName(queryString, cb) {
      let restaurants = (() => {
        ...
        for (var i = 0; i < this.mock_all.list.length; i++) {
          restaurants.push({ value: '' });
          restaurants[i].value = this.mock_all.list[i].customers_name
        }
        // 去重
        let hash = {};
        restaurants = restaurants.reduce((preVal, curVal) => {
    ...
    //filter
    onFilter() {
      var filterData = this.mock_all.list.filter(item => !this.name_search || item.customers_name.toLowerCase().includes(this.name_search.toLowerCase()))
      filterData = filterData.filter(item => !this.id_search || item.customers_id.toLowerCase().includes(this.id_search.toLowerCase()))

...
export default {
  ...
  computed: {
    ...mapState({
      // 获得菜单列表数据
      mock_all: state => state.format.all,
      changeIndex: state => state.format.mock_all,
      Fromadd: state => state.format.mock_all,
    }),
  },
  methods: {
    // 选择
    ...
    pwdChange(index, row) {
      this.fromtitle = '修改';
      this.$store.commit('format/setChangeIndex', index)

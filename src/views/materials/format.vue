...
export default {
  ...
  methods: {
    ...
    //filter
    onFilter() {
      var filterData = this.mock_all.list.filter(item => !this.name_search || item.format_name.toLowerCase().includes(this.name_search.toLowerCase()))
      filterData = filterData.filter(item => !this.id_search || item.format_id.toLowerCase().includes(this.id_search.toLowerCase()))

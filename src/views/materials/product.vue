...
export default {
  ...
  computed: {
    ...mapState({
      // 获得菜单列表数据
      mock_all: state => state.product.all,
      changeIndex: state => state.product.mock_all,
      Fromadd: state => state.product.mock_all,
    }),
  },
  methods: {
    // 选择
    ...
    pwdChange(index, row) {
      this.fromtitle = '修改';
      this.$store.commit('product/setChangeIndex', index)

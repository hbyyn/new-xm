...
const actions = {
  ...
  async getListAction(context) {
    ...
    list = list.map(item => {
      ...
      newItem.material_repair_begindate = item.materialsRepairBegindate
        ? item.materialsRepairBegindate.replace(/T/, " ")
        : item.materialsRepairBegindate;
      newItem.material_repair_enddate = item.materialsRepairEnddate
        ? item.materialsRepairEnddate.replace(/T/, " ")
        : item.materialsRepairEnddate;
  ...
  async deleteSingleAction({ dispatch }, params) {
    ...
    let result = await request.oDelete(
      api.MATERIALSREPAIR_DELETE_SINGLE_API,
      formDelete
    );
    let data = result.data;
    if (data.statusCode == 10000) {
  ...
  async deleteListAction({ dispatch }, param) {
    ...
    console.log(formDelete);
    let result = await request.oDelete(
      api.MATERIALSREPAIR_DELETE_API,
      formDelete
    );
    let data = result.data;
    if (data.statusCode == 10000) {
